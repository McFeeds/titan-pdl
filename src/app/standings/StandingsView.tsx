...
function TeamLogo({ team }: { team: Pick<TeamStanding, "team_name" | "logo_url"> }) {
  return team.logo_url ? (
    ...
    <img src={team.logo_url} alt="" className="w-6 h-6 rounded object-cover shrink-0" />
  ) : (
    <div className="w-6 h-6 rounded bg-indigo-600/40 shrink-0 flex items-center justify-center text-[10px] font-bold text-indigo-300">
                ...
                >
                  ...
                  <td className="px-2 py-2.5">
                    <div className="flex items-center gap-2 min-w-0">
                      <TeamLogo team={team} />
                      <span className="text-gray-200 text-xs font-medium truncate">{team.team_name}</span>
                    </div>
                  </td>
                  <td className="px-2 py-2.5 text-center font-mono text-xs text-emerald-400 font-semibold">
                    {team.wins}
                  </td>
