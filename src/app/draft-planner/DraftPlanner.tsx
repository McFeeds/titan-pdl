...
function SpriteUrl(dexNumber: number | null, large = false): string {
  if (!dexNumber) return "";
  return large
          ...
          <div className="w-80 shrink-0 flex flex-col gap-2">
            ...
            <div className="mt-4 border-t border-white/10 pt-4 flex flex-col gap-3">
              <div className="flex items-center gap-3">
                <span className="text-sm text-gray-400 shrink-0">Budget</span>
                <input
                  type="number"
                  min={0}
                  value={budget}
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className={overBudget ? "text-red-400 font-bold" : "text-emerald-400 font-semibold"}>
                  Remaining: {pointsRemaining}
                </span>
              </div>
              {overBudget && (
                <p className="text-xs text-red-400">Over budget by {Math.abs(pointsRemaining)} points</p>
              )}
            </div>
          </div>
